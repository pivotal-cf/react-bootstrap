...
describe('Tabs', function () {
  ...
  it('Should not pass className to Nav', function () {
    ...
    let myTabPane = ReactTestUtils.scryRenderedDOMComponentsWithClass(instance, 'tab-pane')[0];
    assert.ok(ReactTestUtils.findRenderedDOMComponentWithClass(instance, 'nav-pills'));
    assert.deepEqual(myTabClass, myTabPane);
  });
  ...
  it('Should pass className, Id, and style to Tabs', function () {
    ...
    let tabs = ReactTestUtils.findRenderedDOMComponentWithClass(instance, 'my-tabs-class');
    assert.ok(tabs);
    assert.deepEqual(tabs.props.style, {opacity: 0.5});
    assert.equal(tabs.props.id, 'my-tabs-id');
  });
});
